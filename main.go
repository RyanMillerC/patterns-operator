...
func main() {
	...
	if err = (&controllers.PatternCatalogSourceReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "PatternCatalogSource")
		os.Exit(1)
	}
	if err = (&controllers.PatternManifestReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "PatternManifest")
		os.Exit(1)
	}
